...
class MultiModalDetectionPredictor(DetectionPredictor):
    ...
    def _refresh_modality(self):
        """
        Refresh modality flags from current args.

        Called before each inference to handle modality changes across predict() calls,
        since model.predict() only updates args (not __init__) on subsequent calls.
        """
        modality = getattr(self.args, 'modality', None)
    ...
    def preprocess(self, im):
        ...
        Returns:
            torch.Tensor: 6-channel tensor [RGB, RGB, RGB, X, X, X] format
        ...
        """
        try:
            # Track input mode for downstream use (postprocess, write_results)
    ...
    def _process_dual_modality(self, im):
        ...
        with channel order [RGB, RGB, RGB, X, X, X] matching training stage.

        Args:
            im (List | torch.Tensor): Dual-modal input data

        Returns:
            torch.Tensor: 6-channel tensor [RGB, RGB, RGB, X, X, X]
        """
        ...
        # Combine modalities: [RGB, RGB, RGB, X, X, X] order (matching training data format)
    ...
    def _process_single_modality(self, im):
        ...
        mod = self.modality.lower() if self.modality else ''
        if mod == 'rgb':
        ...
        elif mod in ['depth', 'thermal', 'ir', 'nir', 'x']:
        ...
        # 组合成6通道tensor: [RGB, RGB, RGB, X, X, X] 顺序（与训练数据格式保持一致）
        combined_tensor = torch.cat([rgb_tensor, x_tensor], dim=1)  # Shape: (B, 6, H, W)
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            # Refresh modality flags (may change between predict() calls)
            self._refresh_modality()
    ...
    def write_results(self, i: int, p: Path, im: torch.Tensor, s: list) -> str:
        ...
        Args:
            ...
            im (torch.Tensor): Preprocessed 6-channel tensor [RGB,RGB,RGB,X,X,X]
